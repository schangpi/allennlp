    ...
    def _create_batches(self, instances: Iterable[Instance], shuffle: bool) -> Iterable[Batch]:
        ...
        for instance_list in self._memory_sized_lists(instances):
            ...
            for i, tsk in enumerate(self._tasks):
                ...
                if i == 0:
                    all_batch_instances += lazy_groups_of(iterator[tsk], batch_sizes[tsk])
                else:
                    for j, batch_instances in enumerate(lazy_groups_of(iterator[tsk], batch_sizes[tsk])):
                        all_batch_instances[j] += batch_instances

            # Then break each memory-sized list into batches.
            # for batch_instances in lazy_groups_of(iterator, self._batch_size):
            #     yield Batch(batch_instances)
            for batch_instances in all_batch_instances:
                # embed()
                yield Batch(batch_instances)

    @classmethod
    def from_params(cls, params: Params) -> 'MultiIterator':
