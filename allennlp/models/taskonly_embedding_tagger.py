...
@Model.register("taskonly_embedding_tagger")
class TaskOnlyEmbeddingTagger(Model):
    """
    This ``TaskOnlyEmbeddingTagger`` encodes task token, domain token, and a sequence of text with
    ...
    def forward(self,  # type: ignore
        ...
        *_tags : torch.LongTensor, optional (default = None)
        ...
        concat_text = torch.cat([encoded_text,
                                 embedded_task.expand(-1, num_encoded_vectors, -1)], 2)
    ...
    @classmethod
    def from_params(cls, vocab: Vocabulary, params: Params) -> 'TaskOnlyEmbeddingTagger':
