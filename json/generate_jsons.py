                        ...
                        for c5 in choices[fields[5]]:
                            ...
                            for c6 in choices[fields[6]]:
                                ...
                                for c7 in choices[fields[7]]:
                                    ...
                                    cnt += 1
                                    print(cnt)
                                    print(' '.join([str(decode(data, key)) for key in fields]))
                                    file_ext = '_'.join([str(c) for c in [c0, c1, c2, c3, c4, c5, c6, c7]])
                                    print(file_ext)
                                    # with open('tagcrf_' + file_ext +'.json', 'w') as fw:
                                    #     json.dump(data, fw)
                                    command = ('mn=' + 'tagcrf_' + file_ext + '\n' +
                                               'gpu=' + str(pemb_gpu[c0]) + '\n' +
...
with open('tagcrf_all_commands.txt', 'w') as fw:
    fw.write('\n\n'.join(all_commands))